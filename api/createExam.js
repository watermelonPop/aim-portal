...
export default async function handler(req, res) {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        try {
            ...
            // Extract and properly parse fields
            ...
            const arrStudentIds = studentIds[0].split(",");
            console.log("Student Ids: ", arrStudentIds);
            const parsedCourseId = Number(courseId[0]);

    
            // Fetch a random advisorId
            const advisorId = await getRandomAdvisorId();
    
            // Create an exam for each student
            const newExams = await Promise.all(arrStudentIds.map(studentId =>
                prisma.exam.create({
                    data: {
                        name: examName,
                        ...
                        studentIds: [Number(studentId)], // Prisma expects an array of student IDs
                        courseId: parsedCourseId,
                        advisorId: advisorId, // Directly reference advisorId
                    },
                })
            ));
    
            res.status(201).json(newExams);
        } catch (error) {
